...
helm install cert-manager jetstack/cert-manager --namespace "${LBC_NAMESPACE}"  --version v1.19.2 --set crds.enabled=true --set serviceAccount.create=false --set serviceAccount.name=default --set automountServiceAccountToken=true --set startupapicheck.timeout=5m  --no-hooks 
